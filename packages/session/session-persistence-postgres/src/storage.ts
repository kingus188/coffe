/**
 * The PostgreSQL provider's session storage runtime: its concrete read/write
 * handle with a per-handle mutation chain and a routed live write-behind
 * buffer, the in-process bookkeeping that enforces one active writer handle
 * per session id in this process, and the backend's live event routing and
 * teardown. Deliberately provider-local, mirroring the JSONL backend's
 * runtime shape: the persistence seam exposes only the service and handle
 * contracts, and the shared contract suites
 * (`@deepseek-ai/dsh-session-persistence/tests`) pin equivalent observable
 * behavior across providers. Unlike JSONL, cross-process single-writer
 * exclusion comes from a Postgres advisory lock ({@link AdvisoryLock}), not a
 * kernel file lock — the reason this backend supports multiple application
...
import type { AdvisoryLock } from './lock.ts'

/** Maximum intentional wait before a routed live session batch starts writing. */
export const LIVE_WRITE_BATCH_MAX_DELAY_MS = 200

/** The Postgres-backed storage primitives the handle drives on its owning service. */
export interface PostgresHandleStorage {
  /** Insert (materializing) or append (already durable) one validated batch, bumping revision/event_count in one transaction. */
  persistBatch(
  ...
  /** Read, fail-closed validate, and freeze one `[offset, offset+length)` slice of the durable log. */
  readSlice(header: SessionHeader, offset: number, length: number, signal?: AbortSignal): Promise<SessionHandleReadResult>
  /** Load the complete durable log, fail-closed validating every event, and return its length as the write cursor. */
  validateAndCountLog(header: SessionHeader, signal?: AbortSignal): Promise<number>
  /** Whether the id is still a created-but-unmaterialized session in this process. */
  hasPendingSession(id: SessionId): boolean
  /** Take the session-scoped advisory lock guarding this session across every application instance. */
  acquireWriteLock(header: SessionHeader): Promise<AdvisoryLock>
...
/**
 * The Postgres session handle. Mutations serialize on a per-handle promise
 * chain; reads query the durable log on demand. Routed live events buffer in
 * a bounded window and drain through the same chain as explicit appends.
 */
export class PostgresSessionHandle implements SessionHandle {
  ...
  constructor(
    private readonly storage: PostgresHandleStorage,
    readonly id: SessionId,
    readonly header: SessionHeader,
    readonly access: SessionAccess,
    private readonly state: StorageHandleState,
    /** The cross-process advisory lock; a create handle acquires it lazily at first materialization. */
    private lock?: AdvisoryLock,
  ) {}
  ...
  async read(offset = 0, length = Number.MAX_SAFE_INTEGER, options?: SessionHandleReadOptions): Promise<SessionHandleReadResult> {
    ...
    return this.storage.readSlice(this.header, offset, length, options?.signal)
  }
   ...
   * unwinds first; a drain or lock-release failure still frees the
   * in-process claim, then rejects — both failures together reject as one
   * `AggregateError`.
  ...
  close(): Promise<void> {
    return this.closing ??= (async () => {
      ...
      await this.chain
      // Free the in-process claim no matter how the advisory-unlock fares: a
      // skipped releaseHandle would wedge the id in this process behind a
      // lock Postgres may already have dropped (connection loss).
      ...
      if (failures.length > 1) throw new AggregateError(failures, `session "${this.id}": close failed to drain and to release its advisory lock`)
  ...
  /** The shared durable-append body: contiguity, ownership, storage write, state advance. */
  ...
  /**
   * Hold the cross-process advisory lock before this session's first durable
   * write. An open write handle holds it from construction; a create handle
   * acquires it here — immediately before the first row publishes — and
  ...
  private async ensureLock(): Promise<void> {
    this.lock ??= await this.storage.acquireWriteLock(this.header)
  }
...
/**
 * The Postgres backend's in-process bookkeeping: the single active writer
 * per session id (doubling as the live event router), the open-handle set
 * the teardown sweep closes, and the created-but-unmaterialized sessions this
 * process can already observe. Mirrors the JSONL backend's tracker: this
 * bookkeeping is provider-local by design (see {@link module}), and every
 * backend duplicates the equivalent shape rather than sharing one
 * implementation across physically different storage engines.
 */
export class PostgresBackendTracker {
  ...
  private readonly writers = new Map<SessionId, PostgresSessionHandle | null>()
   ...
   * to open; the handle takes the cross-process advisory lock at its first
   * materializing write.
   ...
   */
  adopt(handle: PostgresSessionHandle): PostgresSessionHandle {
   ...
   */
  release(handle: PostgresSessionHandle, materialized: boolean): void {
  ...
  /**
   * Install the backend's live session routing. Persistence enforces one
   * active write handle per id, so the listeners route published sessions'
   * events by id. Deliberately registers no teardown effect of its own: the
   * owning service's pool must not end until every open handle has drained
   * and closed, and Cordis disposes independent top-level effects
   * concurrently (`Promise.all`), not in registration order — so the owner
   * calls {@link closeOpenHandles} itself, sequenced before it ends the
   * pool, inside its own single effect.
  ...
  install(ctx: Context): void {
    ctx.on('session/event', (session: Session, event) => {
      this.writers.get(session.id)?.enqueueLive(event, (error) => {
        ctx.logger.warn(`session-persistence-postgres: background write for session "${session.id}" failed (buffered events retained): ${String(error)}`)
      })
    })
    ...
    ctx.on('session/disposed', (session: Session) => {
      ...
      writer.close().catch((error: unknown) => {
        ctx.logger.warn(`session-persistence-postgres: final drain for session "${session.id}" failed: ${String(error)}`)
      })
    })
  }

  /**
   * Close every open handle — close drains each one's routed buffer — and
   * aggregate the failures. The owning service's teardown effect awaits this
   * BEFORE ending the connection pool the drains write through.
   * @throws {AggregateError} naming each handle whose close failed.
   */
