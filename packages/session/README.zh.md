...
| [`session-persistence-postgres/`](session-persistence-postgres/README.zh.md) | PostgreSQL 后端：持久事件与可查询消息，通过跨进程写入所有权支持多实例部署；不提供历史格式迁移 | 注册到 `ctx.sessionPersistence` |
