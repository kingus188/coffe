...
| [`session-persistence-postgres/`](session-persistence-postgres/README.md) | PostgreSQL backend: durable events and queryable messages with cross-process write ownership for multi-instance deployments; no historical format migration | registers on `ctx.sessionPersistence` |
